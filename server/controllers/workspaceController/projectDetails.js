...
export const handleGetWorkspaceDashboard = async (req, res) => {
    try {
        ...
        const [workspace, tasks, activities] = await Promise.all([
        ...
        const todoCount = tasks.filter((task) => toStatusKey(task.status) === "todo").length;
        const inProgressCount = tasks.filter((task) => toStatusKey(task.status) === "in-progress").length;
        const doneCount = tasks.filter((task) => toStatusKey(task.status) === "done").length;
        const pendingCount = Math.max(taskCount - (todoCount + inProgressCount + doneCount), 0);

        const today = new Date();
        const overdueCount = tasks.filter((task) => {
            if (!task.dueDate || toStatusKey(task.status) === "done") return false;
            return new Date(task.dueDate) < today;
        }).length;

        const completionRate = taskCount > 0 ? Math.round((doneCount / taskCount) * 100) : 0;
        const dueDate = workspace.dueDate ? new Date(workspace.dueDate) : null;

        const memberTaskCounts = workspace.members.map((member) => {
            const memberId = member.user?._id?.toString?.() || member.user?.toString?.();
            const assignedTasks = tasks.filter((task) => task.assignedTo?._id?.toString?.() === memberId).length;
